...
import random, SocketServer, thread
from datetime import datetime, timedelta
from xmlrpclib import Binary

class myThread (threading.Thread):
    def __init__(self, threadID, name, addr, key, value, end):
        ...
        self.key = key
        self.value = value
        self.end = pickle.dumps(end)
    def run(self):
        #print "Starting " + self.name
        duput(self.name, self.addr, self.key, self.value, self.end)
        #print "Exiting " + self.name

def duput(threadName, addr, key, value, end):
    rpc = xmlrpclib.Server(addr)
    return rpc.putcopy(key, value, Binary(end))
...
class SimpleHT():
  ...
  def put(self, key, value, ttl):
    ...
    mutex.acquire()
    data[key.data] = (value.data, end)
    mutex.release()
    addr = self.addr
    threadc = myThread(1, "DupThread", addr, key, value, end)
    threadc.start()
    return True
  
  def putcopy(self, key, value, end):
    # Remove expired entries
    self.check()
    #print 'putcopy', key
    mutex.acquire()
    data[key.data] = (value.data, pickle.loads(end.data))
    mutex.release()
    return True
...
def main():
  optlist, args = getopt.getopt(sys.argv[1:], "", ["port=", "addr=", "test"])
  ol={}
  for k,v in optlist:
    ol[k] = v
...
def serve(port, addr):
  file_server = RPCThreading(("localhost", port))
  server_object = SimpleHT(addr)
  file_server.register_instance(server_object)
